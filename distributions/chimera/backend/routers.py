...
from modules.agents.backend.routers import agent_discovery_router, agent_installations_router
...
def mount_routers(app: FastAPI, *, prefix: str = "/api/v1") -> None:
    ...
    if "agents" in enabled_modules:
        app.include_router(agent_discovery_router, prefix=prefix)
        app.include_router(agent_installations_router, prefix=prefix)
    ...
    if "workflows" in enabled_modules:
        ...
        if "tools" in enabled_features:
            app.include_router(tools_router, prefix=prefix)
        if "ratings" in enabled_features:
