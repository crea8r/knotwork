...
from core.api.bootstrap.health import (
    initialize_health_state,
    load_or_create_installation_id,
    read_schema_version,
    register_health_route,
)
from core.api.bootstrap.distribution import get_active_backend_distribution
from core.mcp.contracts.bootstrap import bootstrap_mcp_contracts
from core.mcp.server import build_server
from core.api.bootstrap.router import mount_routers
...
def create_app() -> FastAPI:
    snapshot_info = bootstrap_mcp_contracts(distribution_code=_active_distribution_code)
    _kw_logger.info(
        "MCP contract snapshot written to %s (sha256=%s)",
