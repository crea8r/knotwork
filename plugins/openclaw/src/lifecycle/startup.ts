...
import { readPersistedCredentials, readPersistedState } from '../state/persist'
import { acquireRuntimeLease } from '../state/lease'
import { runHandshake, scheduleHandshakeRetry } from './handshake'
import { isOperatorScopeError } from '../openclaw/session'
import { resolveInstanceId } from '../openclaw/bridge'
import type { TimerRef } from './handshake'
...
export type StartupDeps = {
  ...
  hCtx: { state: PluginState; api: OpenClawApi; log: (msg: string) => void; rememberError: (err: unknown) => string; persistState: () => Promise<void> }
  timerRef: TimerRef
...
export function startHydration(deps: StartupDeps): void {
  const { state, cfg, log, rememberError, persistSnapshot, hCtx, timerRef, stateFilePath, credentialsFilePath, lockPath, lockOwnerRef, onHydrated } = deps
  ...
  readPersistedState(stateFilePath)
    .then(async (persisted) => {
      ...
      const creds = await readPersistedCredentials(credentialsFilePath)
