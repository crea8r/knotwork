from __future__ import annotations

from datetime import datetime, timezone
from uuid import UUID, uuid4

from sqlalchemy import select
from sqlalchemy.ext.asyncio import AsyncSession

from core.api import channels as core_channels
from modules.communication.backend.channels_models import Channel, ChannelMessage
from modules.communication.backend.channels_schemas import ChannelCreate, ChannelMessageCreate, DecisionEventCreate

from . import knowledge_folder_service, knowledge_service
from .knowledge_models import KnowledgeChange


def _change_channel_name(path: str) -> str:
    label = path.strip() or "knowledge change"
    return f"review: {label}"


def _proposal_message_metadata(
    *,
    change_id: UUID,
    path: str,
    reason: str,
    proposed_content: str,
    action_type: str,
    target_type: str,
    source_channel_id: UUID | None,
    status: str,
    inline_review: bool,
    payload: dict | None = None,
) -> dict:
    return {
        "kind": "knowledge_change_created",
        "discussion_kind": "review",
        "proposal_id": str(change_id),
        "path": path,
        "reason": reason,
        "proposed_content": proposed_content,
        "action_type": action_type,
        "target_type": target_type,
        "source_channel_id": str(source_channel_id) if source_channel_id else None,
        "status": status,
        "inline_review": inline_review,
        "payload": payload or {},
    }


async def update_inline_proposal_message(
    db: AsyncSession,
    *,
    channel_id: UUID,
    proposal_id: UUID,
    updates: dict,
) -> None:
    rows = await db.execute(
        select(ChannelMessage)
        .where(ChannelMessage.channel_id == channel_id)
        .order_by(ChannelMessage.created_at.desc())
    )
    for message in rows.scalars():
        metadata = dict(message.metadata_ or {})
        if metadata.get("kind") != "knowledge_change_created":
            continue
        if str(metadata.get("proposal_id")) != str(proposal_id):
            continue
        metadata.update(updates)
        metadata["updated_at"] = datetime.now(timezone.utc).isoformat()
        message.metadata_ = metadata
        await db.flush()
        return


async def create_knowledge_change(
    db: AsyncSession,
    *,
    workspace_id: UUID,
    path: str,
    proposed_content: str,
    reason: str,
    run_id: str | None,
    node_id: str | None,
    project_id: UUID | None = None,
    agent_ref: str | None = None,
    source_channel_id: UUID | None = None,
    action_type: str = "update_content",
    target_type: str = "file",
    payload: dict | None = None,
) -> KnowledgeChange:
    inline_review = source_channel_id is not None
    if inline_review:
        channel = await db.get(Channel, source_channel_id)
        if channel is None or channel.workspace_id != workspace_id:
            inline_review = False
    if not inline_review:
        channel = await core_channels.create_channel(
            db,
            workspace_id,
            ChannelCreate(
                name=_change_channel_name(path),
                channel_type="normal",
            ),
        )

    change = KnowledgeChange(
        id=uuid4(),
        workspace_id=workspace_id,
        project_id=project_id,
        run_id=run_id,
        node_id=node_id,
        channel_id=channel.id,
        agent_ref=agent_ref,
        action_type=action_type,
        target_type=target_type,
        target_path=path,
        proposed_content=proposed_content,
        payload=payload or {},
        reason=reason,
        status="pending",
    )
    db.add(change)
    await db.flush()

    await core_channels.create_message(
        db,
        workspace_id,
        channel.id,
        ChannelMessageCreate(
            role="system",
            author_type="system",
            author_name="Knotwork",
            content=(
                f"Proposed a knowledge change for `{path}`."
                if inline_review
                else (
                    f"Knowledge change proposed for `{path}`.\n\n"
                    f"Reason:\n{reason}\n\n"
                    f"Proposed content:\n\n{proposed_content}"
                )
            ),
            run_id=run_id or None,
            node_id=node_id or None,
            metadata=_proposal_message_metadata(
                change_id=change.id,
                path=path,
                reason=reason,
                proposed_content=proposed_content,
                action_type=action_type,
                target_type=target_type,
                source_channel_id=source_channel_id,
                status="pending",
                inline_review=inline_review,
                payload=payload,
            ),
        ),
    )
    await core_channels.create_decision(
        db,
        workspace_id,
        channel.id,
        DecisionEventCreate(
            decision_type="knowledge_change",
            actor_type="agent" if agent_ref else "system",
            actor_name=agent_ref or "Knotwork",
            run_id=run_id or None,
            payload={
                "proposal_id": str(change.id),
                "path": path,
                "reason": reason,
                "proposed_content": proposed_content,
                "action_type": action_type,
                "target_type": target_type,
                "payload": payload or {},
                "source_channel_id": str(source_channel_id) if source_channel_id else None,
            },
        ),
    )

    if target_type == "file":
        existing = await knowledge_service.get_file_by_path(db, workspace_id, path, project_id=project_id)
        if existing is not None:
            project_id = existing.project_id
            change.project_id = existing.project_id
            if not inline_review:
                try:
                    await core_channels.attach_asset_to_channel(
                        db,
                        workspace_id,
                        channel.id,
                        asset_type="file",
                        asset_id=str(existing.id),
                    )
                except ValueError:
                    pass
    elif target_type == "folder":
        folder = await knowledge_folder_service.create_folder(db, workspace_id, path, project_id=project_id)
        if folder is not None:
            project_id = folder.project_id
            change.project_id = folder.project_id
            if not inline_review:
                try:
                    await core_channels.attach_asset_to_channel(
                        db,
                        workspace_id,
                        channel.id,
                        asset_type="folder",
                        asset_id=str(folder.id),
                    )
                except ValueError:
                    pass

    await db.commit()
    await db.refresh(change)
    return change
