...
async def delete_folder(
    ...
    storage_key = str(workspace_id) if project_id is None else f"{workspace_id}:project:{project_id}"
    prefix = path + "/"
    ...
    for f in files_result.scalars().all():
        if f.path == path or f.path.startswith(prefix):
            try:
                await adapter.delete(storage_key, f.path)
            except Exception:
                pass
            await db.delete(f)
...
async def rename_folder(
    ...
    for f in files_result.scalars().all():
        if f.path.startswith(prefix):
            ...
            try:
                await adapter.move(storage_key, f.path, new_file_path, "system")
            except Exception:
                pass
