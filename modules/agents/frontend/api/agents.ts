...
export interface AgentHarnessInstallOption {
  key: string
  display_name: string
  install_kind: string
  mcp_transport: string
  supports_workspace_discovery: boolean
...
export interface AgentHarnessCatalog {
  workspace_id: string
  harnesses: AgentHarnessInstallOption[]
}
...
export function useAgentHarnessCatalog() {
  const workspaceId = useWorkspaceId()
  return useQuery<AgentHarnessCatalog>({
    queryKey: ['agent-harnesses', workspaceId],
    queryFn: async () => {
      const { data } = await api.get(`/workspaces/${workspaceId}/agent-harnesses`)
      return data
    },
    enabled: !!workspaceId,
  })
