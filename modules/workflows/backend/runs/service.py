...
from core.api import runtime as core_runtime
...
from .id import generate_run_id  # noqa: F401 — re-export for callers
from .models import ProviderCallLog, Run, RunHandbookProposal, RunNodeState, RunWorklogEntry
...
async def _collect_run_participant_ids(
    ...
    required_refs: dict[str, tuple[str, str]] = {}
    for node in (definition or {}).get("nodes", []):
        if not isinstance(node, dict) or node.get("type") != "agent":
            continue
        node_name = str(node.get("name") or node.get("id") or "agent-node")
        operator_id = str(node.get("operator_id") or "").strip()
        supervisor_id = str(node.get("supervisor_id") or "").strip()
        if not operator_id:
            raise ValueError(f'Node "{node_name}" is missing an operator')
        if not supervisor_id:
            raise ValueError(f'Node "{node_name}" is missing a supervisor')
        candidate_ids.extend([operator_id, supervisor_id])
        required_refs[node_name] = (operator_id, supervisor_id)

    resolved = set(await resolve_participant_ids(db, workspace_id, candidate_ids))
    for node_name, (operator_id, supervisor_id) in required_refs.items():
        if operator_id not in resolved:
            raise ValueError(f'Node "{node_name}" operator "{operator_id}" is not a valid workspace participant')
        if supervisor_id not in resolved:
            raise ValueError(f'Node "{node_name}" supervisor "{supervisor_id}" is not a valid workspace participant')
    return resolved
...
async def create_run(
    ...
    if selected_graph_version_id is not None:
        ...
        if graph.production_version_id is not None:
            ...
            version = await core_graphs.get_latest_version(db, graph_id)
        if version is None:
            # Fall back to any draft
            version = await core_graphs.get_any_draft(db, graph_id)
            if version is None:
    ...
    errors = core_runtime.validate_graph(version.definition)
    if errors:
    ...
    async def attach_and_announce(
        ...
        *,
        ...
        if target_channel_id == run_channel.id or target_channel_id in announced_channel_ids:
            return
        target_channel = await core_channels.get_channel(db, workspace_id, target_channel_id)
        if target_channel is None:
            return
        if target_channel.channel_type == "consultation":
            return
        await core_channels.create_message(
            db,
            workspace_id,
    ...
    if data.source_channel_id is not None:
        source_channel = await core_channels.get_channel(db, workspace_id, data.source_channel_id)
    ...
    bound_channel_ids = await core_channels.list_bound_channel_ids_for_asset(
        db,
        workspace_id,
        asset_type="workflow",
...
async def clone_run_as_draft(db: AsyncSession, run_id: str) -> Run:
    ...
    version = await core_graphs.get_latest_version(db, source.graph_id)
    if not version:
...
async def delete_run(db: AsyncSession, run_id: str) -> None:
    ...
    from modules.communication.backend.channels_models import ChannelMessage, DecisionEvent
    from .escalations_models import Escalation
    from modules.communication.backend.notifications_models import NotificationLog
