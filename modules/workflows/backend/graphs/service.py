...
from .schemas import GraphCreate, GraphUpdate, GraphVersionCreate
...
async def list_workspace_graphs(db: AsyncSession, workspace_id: UUID) -> list[Graph]:
    result = await db.execute(
        select(Graph).where(Graph.workspace_id == workspace_id).order_by(Graph.created_at.desc())
    )
    return list(result.scalars())
...
async def save_version(
    ...
    from .version_service import _make_version_id
