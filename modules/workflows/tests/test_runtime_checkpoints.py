...
from modules.workflows.backend.runtime.checkpoints import evaluate_checkpoints
...
def test_passing_checkpoint():
    assert evaluate_checkpoints([PASS_CP], {"score": 0.9}) == []
...
def test_human_checkpoint_skipped():
    assert evaluate_checkpoints([HUMAN_CP], {}) == []


def test_mix_pass_fail():
    result = evaluate_checkpoints([PASS_CP, FAIL_CP], {"score": 0.9, "text": ""})
    assert len(result) == 1
    assert result[0]["id"] == "cp2"


def test_all_fail():
    result = evaluate_checkpoints([PASS_CP, FAIL_CP], {"score": 0.3, "text": ""})
    assert len(result) == 2


def test_invalid_expression_fails_safe():
    result = evaluate_checkpoints(
        [{"id": "bad", "type": "expression", "expression": "INVALID%%%", "fail_message": ""}],
        {},
    )
    assert len(result) == 1
...
def test_empty_expression_fails_safe():
    result = evaluate_checkpoints(
        [{"id": "empty", "type": "expression", "expression": "", "fail_message": ""}],
        {},
    )
    assert len(result) == 1
...
def test_human_among_expressions():
    result = evaluate_checkpoints([PASS_CP, HUMAN_CP, FAIL_CP], {"score": 0.95, "text": ""})
    assert len(result) == 1
    assert result[0]["id"] == "cp2"
