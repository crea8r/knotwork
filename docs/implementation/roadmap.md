...
| **S12.1** | `[finished: 2026-04-01]` **Unified participant model + agent bridge design.** `RegisteredAgent` table removed — merged into `WorkspaceMember(kind='agent')`. Agent auth via ed25519 challenge-response → 30-day JWT (no API keys stored). `agent_main` channel type removed — no DM-style channels. Agent discovery: `GET /.well-known/agent` (unauthenticated, gives auth endpoints + skills URL + MCP URL), `GET /skills` (personalised markdown), `knotwork://workspace/skills` MCP resource. `agent-bridge/spec/` behavioral protocol: non-preemptive priority queue with channel-rhythm synthetic deadlines, obligation promotion, session management, 5 event types with ACK semantics. Settings → Members: kind filter + "Add agent by public key" form with discovery URL copy. | S12.2 | ✅ Done |
| **S12.2** | `[finished: 2026-04-08]` **Agent bridge layer.** Workspace guide and generated agent context, Settings guide/onboarding copy, OpenClaw bridge rewrite from old task queue to inbox polling, ed25519 auth, guide reload, degraded-state observability, and MCP-backed semantic transport for channel/file/folder/run context. Remaining live-runtime prompt/guideline polish and blocked escalation retests are promoted to S12.4. | S12.3 | ✅ Done |
| **S12.3** | `[finished: 2026-04-08]` **Participant coordination, channel scope, and workload honesty.** Channel participants visible across chat surfaces; explicit invite/subscription behavior; owner remove, member leave, mention-add, run-chat lock, and legacy implicit-all visibility. One AgentZero role per workspace for a human or agent, with avatar indicator and private objective-scoped consultation. Member profiles now carry contribution brief, availability, capacity, status note, commitments, and recent work through UI and MCP. OpenClaw bridge message-response policy prevents agent reply loops. Strict enterprise ACLs remain Phase 2 scope. | S12.4 | ✅ Done — 9 S12.3 tests, 8 MCP tests, 4 bridge tests |
| **S12.4** | **Bridge fixes, prompt optimization, and guidelines.** Follow-up lane for live OpenClaw runtime fixes, semantic prompt/guideline tuning, escalation validation that was blocked during S12.2 retest, duplicate-delivery watch, and retest harness hardening. | — (Phase 1 complete) | Planned |
...
- **Advanced channel permission scoping** — S12.3 introduces visible channel participation and invite/subscription rules. Phase 2 can add fine-grained enterprise ACLs beyond that baseline.
