...
| **S9.3** | **Participant-specific event delivery.** Stable participant identity model for run/chat events (workspace humans and workspace agents only in Phase 1). Agent can address a specific participant. Addressed escalations route to the targeted participant's inbox. Event delivery is participant-specific, not workspace-wide. Participants register communication means per event type. Supported means in S9.3: app, email, OpenClaw plugin. Deep links use `PUBLIC_BASE_URL` when available. Telegram, WhatsApp, external clients, and plugin/MCP separation are deferred. | S10 | Planned |
...
| **S12.1** | **OpenClaw transport upgrade (re-evaluate after MCP split).** Revisit whether the OpenClaw plugin should move from timer-driven HTTP polling to WebSocket after S12 has reduced the plugin to inbound communication only. If still justified, replace polling with WebSocket and add reliability semantics: ACK/retry for terminal events, reconnect/resume on connection loss, automatic credential recovery without manual restart. Auth-mode auto-resolution (`none` / `token` / `password` / `trusted-proxy`) with unified `callGateway()` wrapper and explicit error codes. Degraded-state observability: explicit states, auth diagnostics, queue diagnostics. Deployment guidance for reverse proxy/TLS idle timeouts. | — (Phase 1 complete) | Planned |
| **S12.2** | **Agent invitation and Handbook-file mentions in designer/workflow chat (post-MCP rethink).** Revisit whether agents can be invited or assigned into designer chat and workflow chat, and whether designer chat should support handbook-file mention syntax like `/filename` and `[[filename]]`, after S12/S12.1 have clarified the MCP/plugin split and OpenClaw's role. The design must not assume the pre-MCP OpenClaw execution/runtime model still applies. | — | Planned |
