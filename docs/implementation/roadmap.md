...
| **S12.2** | `[est. start: 2026-03-24 20:50]` **Post-MCP interaction rethink.** Revisit whether agents can be invited or assigned into designer chat and workflow chat, whether designer chat should support handbook-file mention syntax like `/filename` and `[[filename]]`, whether workload-honesty/queue semantics should still live in the OpenClaw plugin at all, and how project-level AI behaviors should work after the MCP/plugin split. This includes project AI status drafting/writing, project progress summaries, and related project-intelligence interaction design. Qualitative project intelligence, such as progress summaries and objective refinements, belongs here rather than in S11; post-S12, agents may already have `get_project_status`, project chat, and the proposal pattern, so a project lieutenant may be expressible as a configured workflow, but that boundary must be designed here rather than assumed earlier. Historical S10 workload-honesty material is preserved under `docs/implementation/S12.2/workload-honesty-spec.md` and `docs/implementation/S12.2/workload-honesty-plan.md` as design input for this rethink. The design must not assume the pre-MCP OpenClaw execution/runtime model still applies. | — | Planned |
