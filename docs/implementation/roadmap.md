...
| **S8** | **Chat-first agent runtime.** Plugin-first OpenClaw connectivity remains, with split chat domains: agent main session chat for preflight, separate workflow chat for design consultation, and run-per-session chat timeline for execution. Capability is derived from chat-visible skills/tools, escalation continuity is preserved in run chat, and run detail uses persisted chat as source of truth. | S8.1 | In progress |
| **S8.1** | JWT auth wiring + API key enforcement (unauthenticated endpoints locked). RBAC (owner vs operator). Workspace + member settings UI. Audit log viewer. E2E tests. | S9 | Planned |
| **S9** | MCP server (SSE + stdio): exposes graphs, runs, escalations, handbook, worklog as MCP tools + resources. Desktop integrations. Docker + deployment guide. Mobile polish. | — (Phase 1 complete) | Planned |
