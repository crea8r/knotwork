...
| **S12.2** | **Agent bridge layer.** Build agent-bridge reference implementation from S12.1 spec (poller, session manager, event dispatcher, state tracker, auth manager). Rewrite OpenClaw plugin (`plugins/openclaw/`) from execution layer to bridge implementation — strip task execution, add notification polling, migrate to API key auth. Transport decision (HTTP polling vs WebSocket). Credential recovery and degraded-state observability. Migration guide for existing plugin users. | S12.3 | Planned |
| **S12.3** | **Agent Zero + Representatives + Workload Honesty.** Agent Zero as optional orchestrator (`role: orchestrator` in WorkspaceMember config), guided onboarding, workspace-wide monitoring. WorkspaceRepresentative model (unified `WorkspaceMember` FK, priority routing). Workload-honesty queue visibility (unclaimed/queued/running/stalled/orphaned) anchored to participants. Historical workload-honesty material preserved under `docs/implementation/S12.3/` as design input. | — (Phase 1 complete) | Planned |
