...
| **S4** | Chat designer agent (graph deltas), node config panel (knowledge / tools / checkpoints / confidence rules / fail-safe), conditional router and tool executor editors | S5 (complete designer UX), S7 (MCP design tools) | ✅ Done — 26 tests |
| **S5** | Operator dashboard (active runs, escalation widget, recent runs), run trigger form, full canvas with live node status overlay, run history, node inspector, escalation inbox, post-run feedback nudges | S6 (trigger point for notifications), S7 (MCP exposes these views) | ✅ Done — 11 tests |
| **S6** | Tool registry CRUD + execution, built-in tools (web.search/DuckDuckGo, web.fetch, http.request, calc), tool_executor node, notification channels (email + Telegram + WhatsApp Phase 1 deep links), notification preferences, notification log | S7 (MCP exposes tools + notification tools) | ✅ Done — 20 tests |
...
| **S6.2** | Run naming + inline rename, delete queued/paused runs, exact graph version in run detail, enriched runs table (tokens/output/needs_attention), full LLM input captured in node inspector (system prompt + user prompt) | S6.3 | ✅ Done — 11 tests |
| **S6.3** | **Critical:** system prompt + knowledge config key alignment (fix silent empty prompts), per-node input sources (preceding node outputs flow into next node's prompt), edit draft run input, "Run now" debounce | S7 | ✅ Done — 13 tests |
