...
import { addRunningTask, removeRunningTask } from './worker'
...
export function spawnExecuteTask(deps: SpawnDeps, task: ExecutionTask, spawnContext: 'poll' | 'rpc'): void {
  const { state, log, api, activeSpawns, taskLog, maxConcurrent, maxGatewayAttempts, saturationRetryMs, integrationSecret, knotworkUrl, taskLogPath } = deps
  ...
  function postFailed(error: string): void {
    const baseUrl = knotworkUrl || getConfig(api).knotworkBackendUrl
    const instanceId = pluginInstanceId || state.pluginInstanceId
    const secret = integrationSecret || state.integrationSecret
    if (!baseUrl || !instanceId || !secret) return
    import('../openclaw/bridge').then(({ postEvent }) => {
      postEvent(baseUrl, instanceId, secret, taskId, 'failed', { error })
  ...
  const trySpawn = (retryReason?: string, retryDelayMs?: number): void => {
    ...
    const subprocessParams = { task, pluginInstanceId, integrationSecret, knotworkUrl, taskLogPath }
    const buf: Buffer[] = []
    const p = spawn('openclaw', ['gateway', 'call', 'knotwork.execute_task', '--params', JSON.stringify(subprocessParams)], {
    ...
    p.on('close', (code) => {
      ...
      if (isGatewayConnectionError(code, output)) {
        ...
        if (saturated) {
          ...
          if (genuineAttempt >= maxGatewayAttempts) {
            ...
            postFailed(`gateway_unavailable after ${genuineAttempt} genuine retries and ${saturationAttempt} saturation retries`)
            return
          }
      ...
      if (code !== 0) {
        ...
        postFailed(`spawn exited code=${code}${output ? `: ${output.slice(0, 300)}` : ''}`)
      } else {
    ...
    p.on('error', (e: Error) => {
      ...
      postFailed(`spawn error: ${e.message}`)
    })
  }
