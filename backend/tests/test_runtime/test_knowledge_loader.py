...
from knotwork.runtime.knowledge_loader import (
    KnowledgeTree,
    extract_wiki_links,
    get_domain,
    is_universal,
...
def make_adapter(files: dict[str, str]):
    ...
    from knotwork.knowledge.storage.adapter import FileContent
...
async def test_loads_root_fragment():
    ...
    with patch("knotwork.runtime.knowledge_loader.get_storage_adapter", return_value=make_adapter(files)):
