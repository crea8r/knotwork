...
Revision ID: 0028
Revises: 0027_drop_openclaw_tables
...
revision = "0028"
down_revision = "0027_drop_openclaw_tables"
branch_labels = None
depends_on = None

# Default guide seeded into every workspace.
# Mirrors DEFAULT_GUIDE_MD in knotwork/workspaces/guide.py — keep in sync.
_DEFAULT_GUIDE = """\
...
def upgrade() -> None:
    op.add_column("workspaces", sa.Column("guide_md", sa.Text(), nullable=True))
    op.add_column("workspaces", sa.Column("guide_version", sa.Integer(), nullable=False, server_default="0"))

    # Seed all existing workspaces with the default guide.
    op.execute(
        sa.text(
            "UPDATE workspaces SET guide_md = :guide, guide_version = 0 WHERE guide_md IS NULL"
        ).bindparams(guide=_DEFAULT_GUIDE)
    )


def downgrade() -> None:
    op.drop_column("workspaces", "guide_version")
    op.drop_column("workspaces", "guide_md")
